...
public class Client {
	...
	public Client() {
		...
		timer_calc = new Timer(40, new timerListener());
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (rtp_packet.PayloadType == 26) {
					...
					/*
					//check if last packet in group came through
					if((FECpacket.getjpeg(rtp_packet.getsequencenumber()-1) == null) && (lostinGrp == 0)) {
						lostinGrp = rtp_packet.getsequencenumber();
						correctable = true;
					}
					*/
					...
					if (correctable == true) {
						...
						if(FECpacket.mediastack[lostinGrp] != null) {
							System.out.println("KORRIGIERT: >>>>>" + lostinGrp + "<<<<<");
						}
					}
	...
	class timerListenerDisp implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				// System.out.println("Play: "+Arrays.toString(FECpacket.getjpeg(disp_count)));
				if (FECpacket.getjpeg(disp_count) != null) {
